...
export default function DashboardPage() {
  ...
  const allAchievements = useMemo(() => [
    ...
    { id: 'author', title: 'Auteur', description: 'Ajouter manuellement une description à une image.', icon: FileText, xp: XP_PER_ACHIEVEMENT, isEligible: (profile: UserProfile, images: ImageMetadata[]) => images?.some(image => !!image.description && !image.generatedByAI), getProgress: null },
    { id: 'futurist', title: 'Futuriste', description: 'Générer une description avec l\'IA.', icon: Wand2, xp: XP_PER_ACHIEVEMENT, isEligible: (profile: UserProfile, images: ImageMetadata[]) => images?.some(image => image.generatedByAI === true), getProgress: null },
    { id: 'curious', title: 'Curieux', description: 'Lire votre premier tip de créateur.', icon: MailOpen, xp: XP_PER_ACHIEVEMENT, isEligible: () => getSeenMessagesCount() >= 1, getProgress: () => ({ current: getSeenMessagesCount(), target: 1 }) },
    { id: 'secret-seeker', title: 'Chercheur de Secrets', description: 'Lire 5 tips de créateur.', icon: KeyRound, xp: XP_PER_ACHIEVEMENT, isEligible: () => getSeenMessagesCount() >= 5, getProgress: () => ({ current: getSeenMessagesCount(), target: 5 }) },
      ...
      <TooltipProvider>
        <div className="container mx-auto p-4 sm:p-6 lg:p-8">
          <div className="w-full max-w-4xl mx-auto space-y-8">
            ...
            <Card className="transition-all duration-200 ease-out hover:shadow-xl hover:-translate-y-0.5 hover:border-primary">
                <CardHeader>
